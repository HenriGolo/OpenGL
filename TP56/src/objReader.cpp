 ...
 */
bool load(const std::string& filename, std::vector<point3d>& vertices, std::vector<face>& mesh,
          std::vector<vec3d>& normals, BoundingBox& bb)
{
    std::string line;
    ...
    while (!objFile.eof())
    {
        ...
        if (line[0] == 'f')
        {
            ...
            normals[t.v3] += normal * angleAtVertex(vertices[t.v3], vertices[t.v1], vertices[t.v2]);
        }
    }
...
std::optional<point3d> parseVertexStringRegex(const std::string& toParse)
{
    ...
    static const std::regex vertex_regex(
        R"(v\s+([+-]?(?:[0-9]*[.])?[0-9]+(?:[eE][-+]?[0-9]+)?)\s+([+-]?(?:[0-9]*[.])?[0-9]+(?:[eE][-+]?[0-9]+)?)\s+([+-]?(?:[0-9]*[.])?[0-9]+(?:[eE][-+]?[0-9]+)?))");
